linael :master, require_auth: true do
  ...
  def reload_action(name)
    raise MessagingException, t.master.not.loaded(name) unless modules.key?(name)
...
module Linael
  class Modules::ModuleList
    ...
    def delete_module_by_name(module_name)
      raise MessagingException, t.master.not.loaded(name) unless key?(module_name)
    ...
    def good_to_add?(module_name, klass)
      raise MessagingException, t.master.not.unloaded if key? module_name
