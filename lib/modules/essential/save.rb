...
linael :save, require_auth: true do
  ...
  def save_act(msg = nil, what = "")
    ...
    if what.empty?
      ...
      list_to_save << what if (what != "master") && mod(what)
    end
  ...
  def load_act(msg = nil, what = "")
    ...
    if what.empty?
      Dir.foreach('save'){ |file| list_to_load << file.gsub(/\.yaml/, "") if file.match?(/\.yaml/) }
    else
      list_to_load << what if (what != "master") && File.exist?("save/#{what}.yaml")
    end
    ...
    list_to_load.each do |mod_name|
      # if already charged
      # stop the mod
      master.del_action(mod_name) if master.modules.key?(mod_name)
